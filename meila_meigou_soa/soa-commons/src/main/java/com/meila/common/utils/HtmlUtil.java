package com.meila.common.utils;
...
public class HtmlUtil {
    ...
    static {
        ...
        TEXT['"'] = StringUtils.HTML_QUOTE.toCharArray(); // 双引号
        TEXT['&'] = StringUtils.HTML_AMP.toCharArray(); // &符
        TEXT['<'] = StringUtils.HTML_LT.toCharArray(); // 小于号
        TEXT['>'] = StringUtils.HTML_GT.toCharArray(); // 大于号
    }
    ...
    public static String restoreEscaped(String htmlStr) {
        ...
        return htmlStr.replace("&#39;", "'").replace(StringUtils.HTML_LT, "<").replace(StringUtils.HTML_GT, ">").replace(StringUtils.HTML_AMP, "&")
            .replace(StringUtils.HTML_QUOTE, "\"").replace(StringUtils.HTML_NBSP, " ");
    }
    ...
    public static String removeHtmlTag(String content, boolean withTagContent, String... tagNames) {
        ...
        for (String tagName : tagNames) {
            ...
            // (?i)表示其后面的表达式忽略大小写
            regex1 = StringUtils.format("(?i)<{}\\s?[^>]*?/>", tagName);
            if (withTagContent) {
                // 标签及其包含内容
                regex2 = StringUtils.format("(?i)(?s)<{}\\s*?[^>]*?>.*?</{}>", tagName, tagName);
            } else {
                // 标签不包含内容
                regex2 = StringUtils.format("(?i)<{}\\s*?[^>]*?>|</{}>", tagName, tagName);
            }

            content = content.replaceAll(regex1, StringUtils.EMPTY) // 自闭标签小写
                .replaceAll(regex2, StringUtils.EMPTY); // 非自闭标签小写
        }
        return content;
    }
    ...
    public static String removeHtmlAttr(String content, String... attrs) {
        ...
        for (String attr : attrs) {
            regex = StringUtils.format("(?i)\\s*{}=([\"']).*?\\1", attr);
    ...
    public static String removeAllHtmlAttr(String content, String... tagNames) {
        String regex = null;
        for (String tagName : tagNames) {
            regex = StringUtils.format("(?i)<{}[^>]*?>", tagName);
            content.replaceAll(regex, StringUtils.format("<{}>", tagName));
        }
        return content;
    }
