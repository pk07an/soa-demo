    ...
    public boolean load(InputStream settingStream) throws IOException {
        ...
        try {
            ...
            while (true) {
                ...
                line = line.trim();
                // 跳过注释行和空行
                if (StringUtils.isBlank(line) || line.startsWith(Setting.COMMENT_FLAG_PRE)) {
                    // 空行和注释忽略
                    continue;
                } else if (line.startsWith(StringUtils.BACKSLASH + Setting.COMMENT_FLAG_PRE)) {
                ...
                if (line.charAt(0) == Setting.GROUP_SURROUND[0] && line.charAt(line.length() - 1) == Setting.GROUP_SURROUND[1]) {
                    ...
                    if (null == group && null != valueSet) {
                        super.put(StringUtils.EMPTY, valueSet);
                    }
