package com.meila.common.setting.dialect;
...
import com.meila.common.exceptions.SettingException;
import com.meila.common.log.XLog;
import com.meila.common.setting.AbsSetting;
import com.meila.common.utils.Conver;
import com.meila.common.utils.FileUtil;
import com.meila.common.utils.ReUtil;
import com.meila.common.utils.StringUtils;
import com.meila.common.utils.URLUtil;
...
public class GroupSetting extends AbsSetting {
    ...
    public GroupSetting(String pathBaseClassLoader, String charset, boolean isUseVariable) {
        if (null == pathBaseClassLoader) {
            pathBaseClassLoader = StringUtils.EMPTY;
        }

        final URL url = URLUtil.getURL(pathBaseClassLoader);
        if (url == null) {
            throw new RuntimeException(StringUtils.format("Can not find Setting file: [{}]", pathBaseClassLoader));
        }
    ...
    public GroupSetting(File configFile, String charset, boolean isUseVariable) {
        ...
        if (url == null) {
            throw new RuntimeException(StringUtils.format("Can not find Setting file: [{}]", configFile.getAbsolutePath()));
        }
        this.init(url, charset, isUseVariable);
    }
    ...
    public GroupSetting(String path, Class<?> clazz, String charset, boolean isUseVariable) {
        ...
        if (url == null) {
            throw new RuntimeException(StringUtils.format("Can not find Setting file: [{}]", path));
        }
        this.init(url, charset, isUseVariable);
    }
    ...
    public void store(String absolutePath) {
        try {
            ...
            throw new RuntimeException(StringUtils.format("Can not find file [{}]!", absolutePath), e);
        } catch (IOException e) {
        ...
        try {
            ...
            for (Method method : methods) {
                ...
                if (methodName.startsWith("set")) {
                    String field = StringUtils.getGeneralField(methodName);
